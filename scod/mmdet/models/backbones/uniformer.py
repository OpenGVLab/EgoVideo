...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.gamma_1 = nn.Parameter(1 * torch.ones((1, dim, 1, 1)), requires_grad=True)
        self.gamma_2 = nn.Parameter(1 * torch.ones((1, dim, 1, 1)), requires_grad=True)
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.gamma_1 = nn.Parameter(1 * torch.ones((dim)), requires_grad=True)
        self.gamma_2 = nn.Parameter(1 * torch.ones((dim)), requires_grad=True)
...
class UniFormer(nn.Module):
    ...
    def __init__(self, layers=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=80, embed_dim=[64, 128, 320, 512],
                 ...
                 pretrained=None, use_checkpoint=False, checkpoint_num=[0, 0, 0, 0],
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # load state_dict
            meg = self.load_state_dict(_state_dict, False)
            print(meg)
