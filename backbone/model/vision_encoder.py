...
class PretrainVisionTransformer(nn.Module):
    ...
    def init_pos_embed(self):
        ...
        if self.sep_pos_embed:
            ...
            # trunc_normal_(self.clip_pos_embed_spatial, std=.02)
            # trunc_normal_(self.clip_pos_embed_temporal, std=.02)
            # trunc_normal_(self.clip_pos_embed_cls, std=.02)
    ...
    def forward(self, x, mask=None, use_image=False):
        ...
        B_origin = B
        # if T != 4:
        #     x = rearrange(x,'b (t1 t2) l c->b t1 t2 l c',t2 = 4)
        #     x = rearrange(x,'b t1 t2 l c->(b t1) t2 l c')
        B,T,L,C = x.shape
        #x = x.view([B, T * L, C])
        ...
        x_vis = x
        #if num_layers != 40:
        #    return x_vis,x_vis,x_vis,x_vis
        # x_vis = rearrange(x_vis,'(b t1) l c->b t1 l c',b=B_origin)
        # x_vis = x_vis.mean(dim=1)
        x_pool_vis = self.clip_projector(x_vis) # TODO 考虑加个pool_norm
