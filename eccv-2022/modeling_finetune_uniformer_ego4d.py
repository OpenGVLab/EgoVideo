...
model_path = '/mnt/petrelfs/chenguo/pretrain_models/uniformer'
model_path = {
...
class Uniformer(nn.Module):
    ...
    def get_pretrained_model(self, pretrained):
        if pretrained:
            checkpoint = torch.load(model_path[pretrained], map_location='cpu')
    ...
    def get_pretrained_checkpoint_file(self, pretrained_file):
        if pretrained_file:
            print("Load Checkpoint:", pretrained_file)
...
def uniformer_base_256_ego4d_finetune(pretrained=False, **kwargs):
    ckpt_file_path = ""  # set uniformer pretrain weights
