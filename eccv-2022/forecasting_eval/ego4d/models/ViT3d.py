...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.box_pos_embed = get_sinusoid_encoding_table(224*224, embed_dim)
    ...
    def forward_features(self, x, proposals, orig_norm_pred_boxes):
        ...
        box_embed = self.get_box_pos_embed(proposals,img_h,img_w)
        x = x.reshape(B, t, h, w, -1).permute(0, 4, 1, 2, 3)  # [b c t h w]
        x_verb,x_ttc = self.ROIHEAD([x], proposals, box_embed)
        return x_verb,x_ttc

    def get_box_pos_embed(self,proposals,h,w):
        if proposals.shape[0] == 0:
            return None
        float_box = proposals[:,1:].clone().to(proposals.device)
        num_boxes,_ = float_box.shape
        # float_box[:,0::2] = (float_box[:,0::2] / w) * 224
        # float_box[:,1::2] = (float_box[:,1::2] / h) * 224
        # float_box = float_box.long()
    ...
    def forward(self, x, boxes, orig_norm_pred_boxes, orig_pred_boxes=None, pred_object_labels=None, pred_object_scores=None):
        if self.training:
            assert x[0].shape[-1] == self.img_size
        proposals = self.pack_boxes(boxes)
        orig_norm_pred_boxes = torch.cat(orig_norm_pred_boxes,0).float()
        pred_verbs, pred_ttcs = self.forward_features(x[0], proposals, orig_norm_pred_boxes)
        if self.training:
...
@MODEL_REGISTRY.register()
def vit_small_patch16_224(pretrained=False, cfg = None, **kwargs):
...
@MODEL_REGISTRY.register()
def vit_base_patch16_384(pretrained=False, cfg = None, **kwargs):
    model = VisionTransformer(
...
@MODEL_REGISTRY.register()
def vit_large_patch16_384(pretrained=False, cfg = None, **kwargs):
    model = VisionTransformer(
...
@MODEL_REGISTRY.register()
def vit_large_patch16_512(pretrained=False, cfg = None, **kwargs):
    model = VisionTransformer(
...
@MODEL_REGISTRY.register()
def vit_huge_patch16_224(pretrained=False, cfg = None, **kwargs):
    model = VisionTransformer(
