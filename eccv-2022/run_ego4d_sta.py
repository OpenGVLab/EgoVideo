...
import utils
from forecasting_eval.ego4d.utils.parser import load_config
import modeling_finetune_sta
...
def get_args():
    ...
    parser.add_argument('--cfg',
                        type=str,
                        default='/mnt/cache/xingsen/videoMAE_kinetics/VideoMAE_ava/short_term_config/VIT3D.yaml',
                        dest='cfg_file')
    parser.add_argument(
        "opts",
    ...
    parser.add_argument('--nb_classes', default=74, type=int,  #
...
def main(args, cfg, ds_init):
    ...
    dataset_train = build_short_term_dataset(cfg=cfg,split='train')
    dataset_val = build_short_term_dataset(cfg=cfg,split='test')
    ...
    if args.eval:
        ...
        validation_verb_epoch(data_loader_val, model, device, args.output_dir, args.start_epoch, log_writer)
        exit(0)
